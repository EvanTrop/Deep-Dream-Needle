...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if np.prod(self._shape) != np.prod(new_shape):
            ...
            raise ValueError("product of shape is inconsistent")
    ...
    def __getitem__(self, idxs):
        ...
        slice_shape = tuple(
            [
                len(range(*s.indices(self._shape[i]))) for i, s in enumerate(idxs)
            ]
        )
        strides = tuple([self._strides[i]*idxs[i].step for i in range(len(idxs))])
        offset = sum(self._strides[i]*idxs[i].start for i in range(len(idxs)))
        return NDArray.make(shape=slice_shape, 
                            strides=strides, 
                            device=self._device, 
                            handle=self._handle, 
                            offset=offset)
    ...
    def sum(self, axis=None):
        view, out = self.compact().reduce_view_out(axis)
    ...
    def flip(self, axes):
        ...
        for i in axes:
            offset += offsets[i]
            strides[i] = -strides[i]
        out = NDArray.make(shape=self._shape, 
    ...
    def pad(self, axes):
        ...
        ### BEGIN YOUR SOLUTION
        pad_shape, idxs = [], []
        for size, (pad_left, pad_right) in zip(self.shape, axes):
            pad_shape.append(size + pad_left + pad_right)
            idxs.append(slice(pad_left, pad_left + size, None))
        out = array(np.zeros(tuple(pad_shape)), device=self.device)
