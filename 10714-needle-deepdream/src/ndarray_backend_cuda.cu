...
__device__ size_t gidToStridedIndex(size_t gid, CudaVec shape, CudaVec strides,
                                                size_t offset) {
  size_t stridedIndex = 0;
  size_t dividend = gid;
  size_t divisor = 1;
  for (size_t i=0; i<shape.size; i++) divisor *= shape.data[i];
  for (size_t i=0; i<shape.size; i++) {
    divisor /= shape.data[i];
    size_t index = dividend / divisor;
    dividend = dividend % divisor;
    stridedIndex += index * strides.data[i];
  }
  return stridedIndex + offset;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN YOUR SOLUTION
  if (gid < size) {
    size_t index_non_compact = gidToStridedIndex(gid, shape, strides, offset);
    out[gid] = a[index_non_compact];
  }
  /// END YOUR SOLUTION
}
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out,
  ...
  if (gid < M*P) {
    ...
    size_t col = gid % P;
    scalar_t val = 0;
    for (size_t i=0; i<N; i++) val += a[row*N + i] * b[i*P + col];
    out[gid] = val;
  }
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  ...
  if (gid < size) {
    ...
    for (size_t i=0; i<reduce_size; i++) val = max(val, a[gid*reduce_size + i]);
    out[gid] = val;
  }
}
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  ...
  if (gid < size) {
    scalar_t val = 0;
    for (size_t i=0; i<reduce_size; i++) val += a[gid*reduce_size + i];
    out[gid] = val;
  }
}
