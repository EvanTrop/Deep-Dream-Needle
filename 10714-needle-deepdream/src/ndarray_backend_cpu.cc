...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  size_t n_dim = shape.size();
  size_t size = 1;
  for (size_t i = 0; i < n_dim; i++) size *= shape[i];
  size_t n = 0;
  std::vector<int32_t> indices(n_dim, 0);
  while (n < size) {
    size_t idx = 0;
    for (size_t i = 0; i < n_dim; i++) idx += strides[i] * indices[i];
    out->ptr[n] = a.ptr[idx + offset];
...
void MatmulTiled(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, uint32_t m,
  ...
  uint32_t m_ = m/TILE, n_ = n/TILE, p_ = p/TILE;
  for (size_t i=0; i<m_; i++) {
    for (size_t k=0; k<p_; k++) {
      for (size_t j=0; j<n_; j++) {
        AlignedDot(a.ptr + (i*n_+j)*TILE*TILE,
                   b.ptr + (j*p_+k)*TILE*TILE,
                   out->ptr + (i*p_+k)*TILE*TILE);
      }
    }
  }
  /// END YOUR SOLUTION
